...
- **G2P (Grapheme-to-Phoneme)**: `DotNetG2PJapaneseG2P`（Pure C#, ネイティブDLL不要）を使用。dot-net-g2p の MeCabTokenizer + G2PEngine で形態素解析・音素変換を行い、HtsLabelParser + ProsodyToneCalculator でトーン計算
...
| `G2PEngine` | `DotNetG2P` | G2Pエンジン。Pure C# 実装（ネイティブDLL依存なし） |
...
### G2P（テキスト→音素変換）
- **DotNetG2PJapaneseG2P (dot-net-g2p)**: Pure C# G2P。MeCabTokenizer + G2PEngine.ToFullContextLabels() → HtsLabelParser → ProsodyToneCalculator.ComputeTonesFromProsody() でトーン計算。ネイティブDLL不要でクロスプラットフォーム対応
