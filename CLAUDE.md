...
- **G2P (Grapheme-to-Phoneme)**: 2つのバックエンドを条件付きコンパイルで切替。`USBV2_DOTNET_G2P_AVAILABLE` 定義時は `DotNetG2PJapaneseG2P`（Pure C#, ネイティブDLL不要）、未定義時は `JapaneseG2P`（OpenJTalk P/Invoke）が使用される
...
| `G2PEngine` | `OpenJTalk` | G2Pエンジン (`OpenJTalk` / `DotNetG2P`)。`USBV2_DOTNET_G2P_AVAILABLE` 定義時は `DotNetG2P` が推奨デフォルト（ネイティブDLL依存なし） |
...
- **DotNetG2PJapaneseG2P (dot-net-g2p)**: Pure C# G2P。MeCabTokenizer + G2PEngine.ToFullContextLabels() → HtsLabelParser → ProsodyToneCalculator.ComputeTonesFromProsody() で既存トーン計算を完全再利用。`#if USBV2_DOTNET_G2P_AVAILABLE` で条件コンパイル
...
### 条件付きコンパイル
- **`USBV2_DOTNET_G2P_AVAILABLE`**: dot-net-g2p UPM パッケージの `versionDefines` により自動定義。手動設定不要
  - **定義時**: `DotNetG2PJapaneseG2P` が有効。`JapaneseG2P` / `OpenJTalkNative` / `OpenJTalkHandle` / `OpenJTalkConstants` はコンパイル対象外
  - **未定義時**: `JapaneseG2P` が有効（OpenJTalk P/Invoke）。`DotNetG2PJapaneseG2P` はコンパイル対象外
  ...
  Runtime/
    Core/
      ...
      TextProcessing/
        ...
        DotNetG2PJapaneseG2P.cs       # dot-net-g2p ベースG2P実装
      Audio/
