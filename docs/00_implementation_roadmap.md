...
| 3 | G2P（OpenJTalk / dot-net-g2p + 音素マッピング） | 3-5日 | High | 0 | Done |
...
## Phase 3: G2P（OpenJTalk / dot-net-g2p + SBV2PhonemeMapper） ✅
...
2つのG2Pバックエンドを条件付きコンパイルで切り替える:

| バックエンド | 条件 | 説明 |
|---|---|---|
| OpenJTalk P/Invoke | `#if !USBV2_DOTNET_G2P_AVAILABLE` | ネイティブDLL依存。Windows x86_64のみ |
| dot-net-g2p Pure C# | `#if USBV2_DOTNET_G2P_AVAILABLE` | Pure C#実装。クロスプラットフォーム対応。**利用可能時はデフォルト** |

`ProsodyToneCalculator` は両バックエンドで共有されるトーン計算ユーティリティ。
...
| `Runtime/Core/TextProcessing/ProsodyToneCalculator.cs` | 共有トーン計算ユーティリティ（両バックエンドで使用） |

**OpenJTalk バックエンド** (`#if !USBV2_DOTNET_G2P_AVAILABLE`):
...
| `Runtime/Core/TextProcessing/JapaneseG2P.cs` | OpenJTalkベースG2Pオーケストレータ |

**dot-net-g2p バックエンド** (`#if USBV2_DOTNET_G2P_AVAILABLE`):

| ファイル | 説明 |
|---|---|
| `Runtime/Core/TextProcessing/DotNetG2PJapaneseG2P.cs` | dot-net-g2pベースG2P実装 |
| `Runtime/Core/TextProcessing/HtsLabelParser.cs` | HTS形式ラベルパーサー |
...
**JapaneseG2P** (OpenJTalkバックエンド):
- 統合パイプライン: TextNormalizer → OpenJTalk P/Invoke → SBV2PhonemeMapper

**DotNetG2PJapaneseG2P** (dot-net-g2pバックエンド):
- 統合パイプライン: TextNormalizer → dot-net-g2p → HtsLabelParser → SBV2PhonemeMapper
- Pure C#実装のためネイティブDLL不要。クロスプラットフォーム対応
- `USBV2_DOTNET_G2P_AVAILABLE` 定義時にデフォルトで使用される
...
| OpenJTalk DLLの不安定性 | Phase 3 ブロック | uPiperの実績あるバイナリを使用。**dot-net-g2p Pure C#バックエンドで軽減済み** |
...
| G2P | `4b0d42e` feat: add dot-net-g2p integration (Phase 0-1) | 完了 |
| G2P | `57b836c` feat: complete Phase 2 tone verification and pipeline integration | 完了 |
| G2P | `3e28603` feat: complete Phase 3-4 tests and Milestone 3 conditional compilation | 完了 |
| G2P | `bf9c1ee` fix: default to DotNetG2P when available in TTSPipelineBuilder | 完了 |
