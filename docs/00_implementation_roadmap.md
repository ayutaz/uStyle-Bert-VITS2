...
| 3 | G2P（dot-net-g2p + 音素マッピング） | 3-5日 | High | 0 | Done |
...
## Phase 3: G2P（dot-net-g2p + SBV2PhonemeMapper） ✅
...
dot-net-g2p (Pure C#) を唯一のG2Pバックエンドとして使用する。ネイティブDLL不要でクロスプラットフォーム対応。
...
| `Runtime/Core/TextProcessing/SBV2PhonemeMapper.cs` | 音素→SBV2トークンIDマッピング |
| `Runtime/Core/TextProcessing/IG2P.cs` | G2Pインターフェース（テスト用モック対応） |
| `Runtime/Core/TextProcessing/G2PResult.cs` | G2P出力のreadonly struct |
| `Runtime/Core/TextProcessing/ProsodyToneCalculator.cs` | トーン計算ユーティリティ |
...
**トーン配列生成**:
- HTS full context labels から韻律情報（A1/A2/A3）を抽出
...
- [x] dot-net-g2p の MeCabTokenizer が辞書をロードできる
- [x] "こんにちは" → 正しい音素列
...
> G2PTests は dot-net-g2p の辞書が必要。PhonemeMapperTests は辞書不要で常時実行可能。
...
| G2P処理精度 | Phase 3 品質低下 | dot-net-g2p Pure C#バックエンド + 200+テストケースで検証済み |
